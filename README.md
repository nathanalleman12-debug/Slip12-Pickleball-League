...
- **Players** — Add players solo or as locked pairs. Toggle "SOLO" / "PAIR" at the top of the Add Player form. You can also pair two existing solo players by clicking the link icon next to each name. Or paste rows from your Google Sheets signup form (format: `Name, Skill, Unit, Partner` per line — Partner column is optional).
- **Teams** — Hit Generate. Locked pairs always stay together; solo players get randomized around them. "Skill-balanced" keeps teams fair. Click team names to rename them. A "PAIR" badge marks teams that signed up together.
